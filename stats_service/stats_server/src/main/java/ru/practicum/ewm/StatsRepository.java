...
public interface StatsRepository extends JpaRepository<EndpointHit, Integer> {
    ...
    @Query("SELECT new ru.practicum.ewm.ViewStats(e.app, e.uri, COUNT(e.ip) AS hits) " +
            "FROM EndpointHit e " +
    ...
    @Query("SELECT new ru.practicum.ewm.ViewStats(e.app, e.uri, COUNT(distinct e.ip) AS hits) " +
            "FROM EndpointHit e " +
            "WHERE e.timestamp BETWEEN :start AND :end " +
            "GROUP BY e.app, e.uri " +
            "ORDER BY hits DESC")
